...
class InternVLAN1AsyncAgent:
    def __init__(self, args):
        ...
        # 默认强制要求 flash-attn 可用：缺失或无法加载 CUDA 扩展时立即报错，不静默回退 eager。
        # 例外：显式设置 INTERNNAV_ALLOW_EAGER_ATTN=1 时允许回退到 eager（用于 flash-attn
        # 尚未编译安装的冷容器上做控制层验证；输出数值与 flash 等价，仅速度不同）。
        allow_eager = os.environ.get("INTERNNAV_ALLOW_EAGER_ATTN", "0") == "1"
        if allow_eager:
            attn_impl = "eager"
            print("[WARN] INTERNNAV_ALLOW_EAGER_ATTN=1，强制 attn_implementation=eager。")
        else:
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "或设置 INTERNNAV_ALLOW_EAGER_ATTN=1 回退到 eager。"
                ) from exc
