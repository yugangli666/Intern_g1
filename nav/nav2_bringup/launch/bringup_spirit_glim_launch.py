...
def generate_launch_description() -> LaunchDescription:
    ...
    declare_graph_file_cmd = DeclareLaunchArgument(
        'graph',
        default_value=os.path.join(bringup_dir, 'graphs', 'spirit_graph.geojson'),
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(bringup_dir, 'params', 'spirit_params.yaml'),
    ...
    declare_enable_stamped_cmd_vel_cmd = DeclareLaunchArgument(
        ...
        default_value='False',
        description='Whether to enable stamped cmd_vel or not',
    )
    ...
    declare_start_nav_executor_cmd = DeclareLaunchArgument(
        'start_nav_executor',
        default_value='True',
        description='Whether to start the RobotNavTaskService compatibility executor',
    )
        ...
        [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(launch_dir, 'glim_launch.py')
                ),
    ...
    nav2_rviz_launch = os.path.join(bringup_dir, 'launch', 'rviz_launch.py')
    start_nav2_rviz = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(nav2_rviz_launch)
    )

    # Launch socket node
    start_socket_node = Node(
        package='control_bridge',
        executable='message_forward',
        name='message_forward'
    )

    start_nav_executor_node = Node(
    ...
    ld.add_action(declare_start_nav_executor_cmd)

    # Add the actions to launch all of the navigation nodes
    ld.add_action(bringup_cmd_group)
    ld.add_action(start_nav2_rviz)
    ld.add_action(start_socket_node)
    ld.add_action(start_nav_executor_node)
